...
module.exports = function(config) {
	...
	// function to get the omit href option
	function getOmitHref(req, jsonOpts) {
		if (_.has(req.params, PARAMS.omithref)) {
			jsonOpts.omitHref = toBoolean(req.params.omithref);
		}
	}
	
	
	// function to get the omit pivot option
	function getOmitPivot(req, jsonOpts) {
		if (_.has(req.params, PARAMS.omitpivot)) {
			jsonOpts.omitPivot = toBoolean(req.params.omitpivot);
		}
	}
	
	// function to get the omit foreign option
	function getOmitForeign(req, jsonOpts) {
		if (_.has(req.params, PARAMS.omitforeign)) {
			jsonOpts.omitForeign = toBoolean(req.params.omitforeign);
		}
	}
	
	// function to get the omit version option
	function getOmitVersion(req, jsonOpts) {
		if (_.has(req.params, PARAMS.omitversion)) {
			jsonOpts.omitVersion = toBoolean(req.params.omitversion);
		}
	}
	...
	function qsParse(req) {
		...
		// jsonOpts setters
		getOmitHref(req, qsp.jsonOpts);
		getOmitPivot(req, qsp.jsonOpts);
		getOmitForeign(req, qsp.jsonOpts);
		getOmitVersion(req, qsp.jsonOpts);
