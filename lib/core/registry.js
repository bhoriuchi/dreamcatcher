...
module.exports = function(config) {
	...
	return function(dream) {
		...
		// function for adding a socket.io listener
		function socket(e) {
			if (_.has(e, 'event') && _.has(e, 'listener') &&
					typeof(e.listener) === 'function' &&
					typeof(e.event) === 'string') {
				...
				// add the listener to the registry
				dream.registry.sockets.push(e);
			}
			else {
				console.log('failed to push socket to the registry');
			}
		}
		
		// add plug-ins. a plug-in must be a function that takes 
		// dreamcatcher as its argument
		// and return an object containing a key named routes that contains
		// an array of routes. the plug-in will be accessible
		// from the registry under the plug-ins key
		function plugin(p) {
			if (typeof(p) === 'function') {
				dream.registry.plugins[p.name] = p(dream);
				return dream.registry.plugins[p.name];
			}
			return null;
		}
