...
module.exports = function(config) {
	...
	function run(routes) {
		...
		config.dream.io = config.io(server.server);
		config.io       = config.dream.io;
		...
		server.listen(port, function() {
			...
			_.forEach(config.registry.sockets, function(event) {
				...
				// check that the event has a valid type
				if (typeof(config.dream.io[event.type]) === 'function') {
					...
					// create the listener
					config.dream.io[event.type](event.event, event.listener);
				}
			});
		});
	}
