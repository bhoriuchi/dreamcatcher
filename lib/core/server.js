...
module.exports = function(config) {
	...
	function run(routes) {
		...
		// initialize socket.io if there are sockets
		if (config.registry.sockets.length > 0) {
			config.io = config.io(server.server);
		}
		...
		server.listen(port, function() {
			...
			// add any socket.io listeners
			_.forEach(config.registry.sockets, function(event) {
				
				// check that the event has a valid type
				if (typeof(config.io[event.type]) === 'function') {
					
					// create a new timestamp
					timestamp = new Date(Date.now()).toISOString();
					
					// log that the event listener has been added
					console.log(timestamp, ': Adding', event.type, 'socket.io listener for', event.event);
					
					// create the listener
					config.io[event.type](event.event, event.listener);
				}
			});
		});
	}
