...
module.exports = function(config) {
	...
	// similar to passport.js the handler should first take in a callback to call
	// on successful authentication and return a function that takes req, res, next
	// as its options that does the actual authentication
		...
		return function(req, res, next) {
			...
			.then(function(validated) {
				...
				if (validated) {
					...
					var msg = {
						details: 'ip address ' + ip + ' was denied access'
					};
					
					msg = _.merge({}, STATUS.UNAUTHORIZED, msg);
					res.send(STATUS.UNAUTHORIZED.code, msg);
					next();
				}
			});
		};
